...
class AwesomeTabBarItem extends Component {
  render() {
    ...
    return (
      <View style={[styles.container]}>
        <Animated.View
          style={{
            flex: 1,
            opacity: this.tabContentAnimatedValue,
        ...
        <View style={[styles.content, { backgroundColor }]}>
          {React.Children.map(children, (child, index) => {
            let Icon = cloneDeep(child.props.icon);
            Icon.props.color = child.props.color || "grey";
            const isActive = activeItem === index;
            if (isActive) {
              this.animate(Easing.ease, index);
              Icon.props.color = child.props.activeColor;
            }
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {isActive && (
                    <Animated.Text style={[activeTextStyle]}>
                      {child.props.title}
                    </Animated.Text>
                  )}
                  {!isActive &&
                    child.props.count &&
                    child.props.count.number > 0 && (
                      <Animated.View
                        style={[
                          styles.count,
                          {
                      ...
                      >
                        <Animated.Text
                          style={[
                            styles.countText,
                            {
                              color: child.props.count.txtColor
                                ? child.props.count.txtColor
                                : "white"
                            }
                          ]}
                        >
                          {child.props.count.number > 99
                            ? "99+"
                            : child.props.count.number}
                        </Animated.Text>
                      </Animated.View>
                    )}
                </Animated.View>
...
AwesomeTabBar.Item = AwesomeTabBarItem;

AwesomeTabBar.defaultProps = {
  backgroundColor: "white"
};

AwesomeTabBar.propTypes = {
  children: PropTypes.arrayOf(PropTypes.element).isRequired,
  onChangeTab: PropTypes.func,
  backgroundColor: PropTypes.string
};

AwesomeTabBarItem.defaultProps = {
  activeColor: "white"
};

AwesomeTabBarItem.propTypes = {
